	...
	public String putData(String jwt, @PathParam("guid") String guid)
	{
		...
		try
		{
			...
			if (dhtResult != null)
			{
				...
				if (dataLastupdate.compareTo(existingDataLastupdate) == 1 || dataTimeout.compareTo(existingDataTimeout) == 1)
				{
				...
				else if (dataLastupdate.compareTo(existingDataLastupdate) == -1 || dataTimeout.compareTo(existingDataTimeout) == -1)
				{
					if (Configuration.getInstance().getStartDatabase() == 1)
					{
						// we check if the Data set is already in the db
